...
def convert_1(x):
    ...
    if 'AU' in x[0]:
        return(1,
               x[1],
               max(0,NR*((1+1)/2)*(1-(1)*np.dot(x[1],parameter_vector_current))
                   +PR*((1-1)/2)*(1-(1)*np.dot(x[1],parameter_vector_current))),
...
def convert_2(x):
    ...
    if x[0] == -1:
        return(x[2],
               (1/2)*x[1]*(PR*(1+1)-NR*(1-1)) if x[3] > 0 else 0)
    elif x[0] == 1:
        return(x[2],
               (1/2)*x[1]*(PR*(1-1)-NR*(1+1)) if x[3] > 0 else 0)
...
get_ra =a.map(get_ratio).reduce(lambda x, y: x + y)
PR = get_ra/numberOfDocs
NR = 1-PR
...
while True:
    ...
    #cost = cost_list[0]+ (1/numberOfDocs*c*2)*(np.square(parameter_vector_current).sum())
    cost = cost_list[0]
    # calculate gradients.
#     if cost > oldCost:
#         break

#     l_gradient_list = cost_list[1]+(1/numberOfDocs*c)*parameter_vector_current
    l_gradient_list = cost_list[1]
